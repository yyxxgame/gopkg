...
func init() {
	tw, err := collection.NewTimingWheel(time.Second, timingWheelSlots, cleanCache)
	logx.Must(err)
	timingWheel.Store(tw)

	proc.AddShutdownListener(func() {
		if err := tw.Drain(cleanCache); err != nil {
			logx.Errorf("failed to drain timing wheel: %v", err)
		}
	})
}

// AddCleanCacheTask adds a clean task on given keys.
func AddCleanCacheTask(task func() error, keys ...string) {
	...
	if err := tw.SetTimer(stringx.Randn(taskKeyLen), delayTask{
		...
		logx.Errorf("failed to set timer for keys: %q, error: %v", strings.Join(keys, ","), err)
	}
}

func cleanCache(key, value any) {
	taskRunner.Schedule(func() {
		...
		if ok {
			...
			msg := fmt.Sprintf("retried but failed to clear cache with keys: %q, error: %v", strings.Join(dt.keys, ","), err)
			logx.Error(msg)
			stat.Report(msg)
		}
	})
